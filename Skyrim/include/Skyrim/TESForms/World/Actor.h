...
public:
	...
	virtual void	UpdateRefractionProperty(UInt8 a1, float refraction);			// C2 006E9780 - updates(?) ninode and adds/removes refraction extra data
	...
	virtual void	UpdateRefractionWithExtraData(void);							// DF 006E9EC0 //calls UpdateRefractionProperty with extraData refraction value at the end
	virtual void	UpdateRefraction(float a1 = 1.0);								// 006E9190
	...
	{
		return (flags1 & kIsPlayerTeammate) != 0;
	}
	...
	DEFINE_MEMBER_FN_const(IsDecapitated, bool, 0x004D5B90, SInt32 unk1);
	...
	enum Flags1 : UInt32
	{
		kAIEnabled = 0x00000002,
		kFlag1_00000100 = 0x00000100,
		kFlag1_00000200 = 0x00000200, // skips most AI processing?
		kFlag1_00000800 = 0x00000800, // cleared on actors that start dead
		kFlag1_00004000 = 0x00004000,
		kFlag1_00008000 = 0x00008000,
		kIsInWater = 0x00040000, // related to the Waterbreathing magic effect. also related to the actor being in cell water, or in lava; checked by a condition meant for the latter
		kFlag1_00200000 = 0x00200000, // skips most AI processing?
		kFlag1_01000000 = 0x01000000,
		kIsPlayerTeammate = 0x04000000, // 1 << 0x1A
		kFlag1_10000000 = 0x10000000, // related to AI
		kIsGuard = 0x40000000,
		kFlag1_80000000 = 0x80000000, // if set, then damage calcs don't bother checking whether the actor is blocking; see 00797799
	};
